...
// ── Render reports page ───────────────────────
function renderR(){
  // Render summary banner + active filter chips first (Phase 2)
  if (typeof renderRptSummary === 'function') renderRptSummary();
  if (typeof updateActiveFilterBar === 'function') updateActiveFilterBar();
  const d=getRD();
...
// ═══════════════════════════════════════════════
// PHASE 2 — Smart presets + collapsible advanced + summary banner
// ═══════════════════════════════════════════════

// Apply a smart preset: resets all filters first, then sets the preset's filters
function applyRptPreset(presetKey, el){
  ...
  RPT.status   = '';
  RPT.priority = '';
  if (typeof aR !== 'undefined' && aR.clear) aR.clear();
  if (typeof aW !== 'undefined' && aW.clear) aW.clear();
  ...
  if (presetKey === 'all') {
    ...
    RPT.priority = 'Urgent';
    RPT.status   = '';
  } else if (presetKey === 'completed-today') {
  ...
  // Update preset chip visual state
  document.querySelectorAll('.rpt-preset').forEach(b => b.classList.remove('on'));
  if (el) el.classList.add('on');

  // Sync hidden chip states (so existing filter logic still works)
  syncAdvancedChipStates();

  // Rebuild dynamic chips (requestor + worktype) so "on" state clears
  if (typeof bChips === 'function') bChips();

  // Re-render
  if (typeof renderR === 'function') renderR();
  if (typeof refreshLucide === 'function') setTimeout(refreshLucide, 0);
}

// Sync the hidden/advanced chip states to match the current RPT state
function syncAdvancedChipStates(){
  // Period chips
  document.querySelectorAll('#rpt-period-chips .chip').forEach(b => {
    b.classList.toggle('on', b.getAttribute('data-period') === RPT.period);
  });
  // Status chips
  document.querySelectorAll('#rpt-status-chips .chip').forEach(b => {
    b.classList.toggle('on', (b.getAttribute('data-status') || '') === (RPT.status || ''));
  });
  // Priority chips
  document.querySelectorAll('#rpt-priority-chips .chip').forEach(b => {
    b.classList.toggle('on', (b.getAttribute('data-prio') || '') === (RPT.priority || ''));
  });
  // Date inputs
  const fromEl = document.getElementById('rpt-from');
  const toEl   = document.getElementById('rpt-to');
  const singleEl = document.getElementById('rpt-single');
  if (fromEl) fromEl.value = RPT.from || '';
  if (toEl)   toEl.value   = RPT.to   || '';
  if (singleEl) singleEl.value = RPT.single || '';
}

// Toggle advanced filters section
function toggleRptAdvanced(){
  const adv = document.getElementById('rpt-advanced');
  const btn = document.getElementById('rpt-adv-toggle');
  if (!adv) return;
...
function clearAllRptFilters(){
  ...
  // Reset preset chips
  document.querySelectorAll('.rpt-preset').forEach(b => b.classList.remove('on'));
  const allPreset = document.querySelector('.rpt-preset[data-preset="all"]');
  if (allPreset) allPreset.classList.add('on');

  syncAdvancedChipStates();

  // Rebuild requestor/worktype chips
  if (typeof bChips === 'function') bChips();
...
// Remove a single active filter (called from × on active chips)
function removeRptFilter(filterType, value){
  if (filterType === 'period')   { RPT.period = 'all'; }
  else if (filterType === 'single')  { RPT.single = ''; }
  else if (filterType === 'status')  { RPT.status = ''; }
  else if (filterType === 'priority'){ RPT.priority = ''; }
  else if (filterType === 'requestor' && typeof aR !== 'undefined') { aR.delete(value); }
  else if (filterType === 'worktype' && typeof aW !== 'undefined')  { aW.delete(value); }

  // Update preset state (deselect all if any filter manually removed)
  document.querySelectorAll('.rpt-preset').forEach(b => b.classList.remove('on'));
...
// Render the "Filtering by: [chip] [chip] [chip]" bar
function updateActiveFilterBar(){
  const bar = document.getElementById('rpt-active-bar');
  const chipsEl = document.getElementById('rpt-active-chips');
  const countEl = document.getElementById('rpt-active-count');
  if (!bar || !chipsEl) return;

  const chips = [];

  // Period (only show if not 'all')
  if (RPT.single) {
    chips.push({ type: 'single', value: RPT.single, icon: 'calendar', text: 'Date: ' + (typeof fd === 'function' ? fd(RPT.single) : RPT.single) });
  } else if (RPT.period && RPT.period !== 'all') {
    const periodNames = { today: 'Today', week: 'Last 7 days', month: 'This month', custom: 'Custom range' };
    chips.push({ type: 'period', value: RPT.period, icon: 'calendar', text: periodNames[RPT.period] || RPT.period });
  }

  // Status
  if (RPT.status) {
    chips.push({ type: 'status', value: RPT.status, icon: 'circle-dot', text: 'Status: ' + RPT.status });
  }

  // Priority
  if (RPT.priority) {
    chips.push({ type: 'priority', value: RPT.priority, icon: 'flag', text: 'Priority: ' + RPT.priority });
  }

  // Requestors
  if (typeof aR !== 'undefined' && aR.size > 0) {
    Array.from(aR).forEach(r => {
      chips.push({ type: 'requestor', value: r, icon: 'user', text: r });
    });
  }

  // Work types
  if (typeof aW !== 'undefined' && aW.size > 0) {
    Array.from(aW).forEach(w => {
      chips.push({ type: 'worktype', value: w, icon: 'briefcase', text: w.replace(/_/g,' ') });
    });
  }
  ...
  chipsEl.innerHTML = chips.map(c => {
    const safeValue = String(c.value).replace(/'/g, "\\'");
    return `<span class="rpt-active-chip">
      <i data-lucide="${c.icon}" style="width:11px;height:11px"></i>
      ${c.text}
      <button class="rpt-chip-x" onclick="removeRptFilter('${c.type}','${safeValue}')" title="Remove">
...
// Render the top summary banner — quick-glance KPIs (Path B analytics)
function renderRptSummary(){
  const wrap = document.getElementById('rpt-summary');
  if (!wrap || typeof getRD !== 'function') return;
  ...
  // Average completion time (only for jobs with both submittedAt and completedAt)
  let avgTime = '—';
  const timed = d.filter(r => r.status === 'Completed' && r.submittedAt && r.completedAt);
  if (timed.length) {
    const totalMs = timed.reduce((sum, r) => sum + (new Date(r.completedAt) - new Date(r.submittedAt)), 0);
    const avgMs   = totalMs / timed.length;
    const avgHrs  = avgMs / 3600000;
    if (avgHrs < 1)        avgTime = Math.max(1, Math.round(avgMs / 60000)) + 'm';
    else if (avgHrs < 24)  avgTime = avgHrs.toFixed(1) + 'h';
    else                   avgTime = (avgHrs / 24).toFixed(1) + 'd';
  }

  wrap.innerHTML = `
    <div class="rpt-sum-metric rpt-sum-featured">
      <div class="rpt-sum-icon"><i data-lucide="bar-chart-3"></i></div>
      <div class="rpt-sum-text">
        <div class="rpt-sum-val">${total}</div>
        <div class="rpt-sum-lbl">Total jobs</div>
      </div>
    </div>
    <div class="rpt-sum-metric">
      <div class="rpt-sum-icon" style="background:rgba(110,190,42,.15);color:#6ebe2a"><i data-lucide="check-circle-2"></i></div>
      <div class="rpt-sum-text">
        <div class="rpt-sum-val">${completed} <span class="rpt-sum-rate">${rate}%</span></div>
        <div class="rpt-sum-lbl">Completed</div>
      </div>
    </div>
    <div class="rpt-sum-metric">
      <div class="rpt-sum-icon" style="background:rgba(85,153,245,.15);color:#5599f5"><i data-lucide="loader"></i></div>
      <div class="rpt-sum-text">
        <div class="rpt-sum-val">${inProg}</div>
        <div class="rpt-sum-lbl">In progress</div>
      </div>
    </div>
    <div class="rpt-sum-metric">
      <div class="rpt-sum-icon" style="background:rgba(232,83,74,.15);color:#e8534a"><i data-lucide="alert-triangle"></i></div>
      <div class="rpt-sum-text">
        <div class="rpt-sum-val">${urgent}</div>
        <div class="rpt-sum-lbl">Urgent open</div>
      </div>
    </div>
    <div class="rpt-sum-metric">
      <div class="rpt-sum-icon" style="background:rgba(168,124,240,.15);color:#a87cf0"><i data-lucide="clock"></i></div>
      <div class="rpt-sum-text">
        <div class="rpt-sum-val">${pending}</div>
        <div class="rpt-sum-lbl">Pending</div>
      </div>
    </div>
    <div class="rpt-sum-metric">
      <div class="rpt-sum-icon" style="background:rgba(45,207,179,.15);color:#2dcfb3"><i data-lucide="timer"></i></div>
      <div class="rpt-sum-text">
        <div class="rpt-sum-val">${avgTime}</div>
        <div class="rpt-sum-lbl">Avg time</div>
      </div>
    </div>
  `;
